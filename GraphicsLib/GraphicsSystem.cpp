...
void GraphicsSystem::addTint(GraphicsBuffer& dest)
{
	al_set_blender(ALLEGRO_ADD, ALLEGRO_ALPHA, ALLEGRO_INVERSE_ALPHA);
	al_draw_tinted_bitmap(dest.getBitmap(), al_map_rgba_f(0, 0, 0, 0.5), 0, 0, 0);
}

void GraphicsSystem::removeTint(GraphicsBuffer& dest)
{
	ALLEGRO_BITMAP* pOldTarget = al_get_target_bitmap();
	al_set_blender(ALLEGRO_ADD, ALLEGRO_ALPHA, ALLEGRO_INVERSE_ALPHA);
	al_draw_tinted_bitmap(dest.getBitmap(), al_map_rgba_f(255, 255, 255, 0.5), 0, 0, 0);
}

void GraphicsSystem::clear()
{
	setBufferToColor(*mpBackBuffer, Color(0, 0, 0, 0));
}

ALLEGRO_COLOR GraphicsSystem::getAllegroColorFromColor(const Color& color)
{
	return al_map_rgba(color.getR(), color.getG(), color.getB(), color.getA());
}
